...
pub trait AppLoader<'a>:
    VerifierClient
    + nonvolatile_storage::NonvolatileStorageClient<'a>
{
...
pub trait Decompressor<'a> {
    ...
    fn decompress_buffer(
        ...
        length: usize,
    ) -> (&'static mut [u8], usize, &'static mut [u8], Option<DalsError>);
...
pub trait Authorizer<'a> {
    fn set_client(&self, client: &'a dyn AppLoader<'a>);
    fn authorize_data(&self, app_flash: usize) -> Option<DalsError>;
}

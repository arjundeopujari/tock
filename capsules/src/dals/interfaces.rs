...
pub trait Decompressor<'a>{
    ...
    fn decompress_buffer(&self, buffer: &'static mut [u8], len: u16) -> Result<(),DecompressionError>;
}
...
pub trait DecompressorClient<'a>{
    ...
    fn decompressed_buffer(&self, decompressed_data_buffer: &'static [u8], len: u16, original_data_buffer: Option<&'static [u8]>);
}
